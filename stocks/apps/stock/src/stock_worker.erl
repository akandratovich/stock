...
%% ru_RU
%% ets:foldl(r), как и оригинальный ets:select делает полный обход таблицы и его сложность пропорциональная размеру таблицы.
%% В нашем случае нужно выбрать данные от From до To. Мой код проходит по ordered_set начиная с To до From с помощью prev/lookup
%% (обратный порядок для того, чтобы избежать необходимости делать реверс). Сложность этого алгоритма линейная от размера отрезка,
%% а не от размера таблицы (судя по документации зависимости должна быть логарифмическая из-за лукапа по ордеред сету, но из-за
%% больших констант алгоритма зависимость от размера таблицы не различима). Этот алгоритм намного эффективнее в случае, когда размер
%% таблицы много превышает размер выбираемого отрезка.
%% Предполагаемое количество данных в таблице в реальной среде зависит от профиля использования. Предположительно - до нескольких
%% (десятков) миллионов - это отрезок данных в несколько лет. Далее использовать разбиение по времени.
%% Текущая реализация плохо подходит для выбора данных за большой отрезок времени с большим масштабом (месяц).
%% Один месяц - это ~44 000 минут, и столько же записей в таблице. Можно соптимизировать, например, хранением дополнительно
%% смерженных данных для масштаба час (или день). Это увечит потребление памяти и время загрузки данных и усложнит алгоритм загрузки.
%% Поскольку, это оптимизация больше не алгоритмического плана, я убрал данный функционал из версии.
